...
The mobile workspace targets Expo SDK 54, matching the current iOS Expo Go client. After pulling an SDK change, align all Expo-managed packages and validate the native configuration before starting Metro:
...
Commit any `package-lock.json` changes produced by the upgrade. For future Expo Go releases, upgrade the project SDK rather than forcing an incompatible client; alternatively, use an EAS development build to control the native runtime version.
